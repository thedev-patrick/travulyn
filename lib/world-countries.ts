/**
 * Static ISO 3166-1 country reference data (name + alpha-2 code) for the admin
 * country picker. Bundled locally rather than fetched from an external API —
 * this data is effectively static, so a live endpoint would just be a network
 * dependency and failure mode for no real benefit.
 */
...
/** Flag emoji is derived from the ISO code (regional indicator symbols), not stored. */
export function flagEmojiFromIso(isoCode: string): string {
